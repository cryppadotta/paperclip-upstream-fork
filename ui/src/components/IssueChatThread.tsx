...
// The draft/reassignment helpers that used to live here moved to
// IssueChatComposerDock.tsx along with the composer itself, so they load with
// the lazily-fetched composer chunk instead of the thread's critical path.
...
export function IssueChatThread({
  ...
  // The assistant-ui runtime now lives inside the lazily-loaded composer dock
  // (IssueChatComposerDock); the thread just supplies the send callback so the
  // submit-scroll bookkeeping stays here alongside the transcript.
  const handleComposerSend = useCallback(
    ({ body, reopen, reassignment }: PaperclipIssueRuntimeSendOptions) => {
          ...
          >
            <Suspense
              fallback={
                <div
                  aria-hidden
                  data-testid="issue-chat-composer-loading"
                  className="min-h-24 rounded-md border border-border/70 bg-background/95 shadow-(--shadow-extract-4)"
                />
              }
            >
              <IssueChatComposerDock
                ref={composerRef}
                messages={messages}
                isRunning={isRunning}
