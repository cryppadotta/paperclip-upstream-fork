...
const CLOSE_READINESS_REQUEST_TIMEOUT_MS = readBoundedIntegerEnv(
  "PAPERCLIP_CLOSE_READINESS_REQUEST_TIMEOUT_MS",
  10_000,
  100,
  120_000,
);

function readBoundedIntegerEnv(key: string, fallback: number, min: number, max: number): number {
  const value = Number(process.env[key]);
  if (!Number.isFinite(value)) return fallback;
  return Math.min(max, Math.max(min, Math.floor(value)));
}

function closeReadinessTenantKey(req: Request, actor: ReturnType<typeof getActorInfo>): string {
  if (req.actor.type === "agent" && req.actor.companyId) {
    return `company:${req.actor.companyId}`;
  }
  if (req.actor.type === "board" && req.actor.companyIds?.length === 1) {
    return `company:${req.actor.companyIds[0]}`;
  }
  return `${actor.actorType}:${actor.actorId}`;
}

function createCloseReadinessRequestDeadline(req: Request, res: Response) {
  ...
  const abortForClient = () => {
    ...
    clientAborted = true;
    controller.abort(new Error("Close-readiness requester disconnected"));
  };
  const timeout = setTimeout(() => {
    timedOut = true;
    controller.abort(new Error("Close-readiness request deadline exceeded"));
  }, CLOSE_READINESS_REQUEST_TIMEOUT_MS);
  timeout.unref?.();
...
export function executionWorkspaceRoutes(db: Db, opts: { pluginWorkerManager?: PluginWorkerManager } = {}) {
  ...
  router.get("/execution-workspaces/:id/close-readiness", async (req, res) => {
    ...
    try {
      releaseDemand = closeReadinessDemandLimiter.acquire({
        workspaceKey: id,
        tenantKey: closeReadinessTenantKey(req, actor),
      });
    } catch (error) {
      res.setHeader("Retry-After", "1");
      throw error;
    }
    const deadline = createCloseReadinessRequestDeadline(req, res);
    try {
      // Authorization needs only persisted metadata. Avoid the hydrated lookup here:
      // it performs the same Git inspection as close readiness and used to double
      // the fan-in generated by every request.
      const workspace = await getAccessibleResource(
        req,
        res,
        svc.getById(id, { inspectGit: false }),
      ...
      const readiness = await svc.getCloseReadiness(id, {
        ...
        fairnessKeys: [`actor:${actor.actorType}:${actor.actorId}`],
      });
      if (deadline.clientAborted) {
        closeReadinessDemandLimiter.recordAborted();
        return;
      }
      if (deadline.timedOut) closeReadinessDemandLimiter.recordTimedOut();
      if (!readiness) {
  ...
  router.patch("/execution-workspaces/:id", validate(updateExecutionWorkspaceSchema), async (req, res) => {
    const id = req.params.id as string;
    // The PATCH path performs its own bounded, cancellation-aware close check
    // below when archiving. Hydrating here would start an unbounded duplicate
    // Git inspection before demand admission.
    ...
    if (req.body.status === "archived" && existing.status !== "archived") {
      ...
      try {
        releaseDemand = closeReadinessDemandLimiter.acquire({
          workspaceKey: existing.id,
          tenantKey: `company:${existing.companyId}`,
        });
      } catch (error) {
        res.setHeader("Retry-After", "1");
        throw error;
      ...
      const deadline = createCloseReadinessRequestDeadline(req, res);
      let readiness: Awaited<ReturnType<typeof svc.getCloseReadiness>>;
      try {
        readiness = await svc.getCloseReadiness(existing.id, {
          signal: deadline.signal,
          fairnessKeys: [`actor:${actor.actorType}:${actor.actorId}`],
          cacheTtlMs: 0,
        });
      ...
      const acknowledgedUnavailableGit = Boolean(
        req.body.acknowledgeGitUnavailable
        && readiness.requiresGitUnavailableAcknowledgement
        && readiness.blockingReasons.length === 1,
      );
      if (readiness.state === "blocked" && !acknowledgedUnavailableGit) {
        res.status(409).json({
      ...
      if (acknowledgedUnavailableGit) {
        cleanupWarnings.push(
          readiness.gitInspection.message
            ? `Git readiness was explicitly acknowledged as unavailable: ${readiness.gitInspection.message}`
            : "Git readiness was explicitly acknowledged as unavailable.",
        );
      }
      ...
      try {
        ...
        if (fenced.skippedReopened) {
          ...
          cleanupWarnings = [...cleanupWarnings, ...cleanupResult.warnings];
