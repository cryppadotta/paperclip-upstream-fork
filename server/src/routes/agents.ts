...
export function agentRoutes(
  ...
  router.get("/companies/:companyId/heartbeat-runs", async (req, res) => {
    ...
    res.json(await Promise.all(runs.map(async (run) => runRedactions.redactForRun(
      companyId,
      run.id,
      await serializeRunListRow(req, run),
    ))));
  });
  ...
  router.get("/companies/:companyId/live-runs", async (req, res) => {
    ...
    if (targetRunCount > 0 && liveRuns.length < targetRunCount) {
      ...
      res.json(await Promise.all(rows.map(async (run) => runRedactions.redactForRun(
        companyId,
        run.id,
        !(await actorCanReadRun(req, run))
          ? redactHeartbeatRunListRow(run)
          : {
    ...
    res.json(await Promise.all(liveRuns.map(async (run) => runRedactions.redactForRun(
      companyId,
      run.id,
      !(await actorCanReadRun(req, run))
        ? redactHeartbeatRunListRow(run)
