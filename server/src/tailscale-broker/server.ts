 ...
 * layered:
 *   1. Filesystem: a root-owned, non-world-writable parent directory and a
 *      `0660` socket owned by `root:<paperclip-group>`. Only members of that
 *      group can open the socket at all. Startup refuses to run if the parent
 *      directory is missing, a symlink, or writable by others.
 ...
 *      Node has no native SO_PEERCRED, so the default reader reports the single
 *      configured service identity the socket is group-restricted to; a native
 *      reader can be injected for stronger per-connection attestation.
...
export function startBrokerServer(options: BrokerServerOptions): net.Server {
  ...
  server.on("connection", (socket) => {
    const peer = options.peerReader.read(socket);
    let received = 0;
    const chunks: Buffer[] = [];
    let finished = false;
  ...
  server.on("error", (err) => onError(`broker server error: ${err.message}`));
  server.listen(options.socketPath, () => {
    try {
      chmodSync(options.socketPath, 0o660);
    } catch (err) {
      onError(`could not chmod broker socket: ${(err as Error).message}`);
    }
  });
  return server;
}
...
/**
 * Default peer-credential reader. Because the socket is group-restricted to the
 * single Paperclip service identity, the connecting peer is that identity by
 * construction; report it so the broker's allowlist still runs. Replace with a
 * native SO_PEERCRED reader for per-connection attestation.
 */
export function fixedPeerCredentialReader(identity: PeerIdentity): PeerCredentialReader {
