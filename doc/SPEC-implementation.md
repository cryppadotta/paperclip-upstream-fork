...
- the source fingerprint, source-attempt count, next due time, source owner, and return owner persist in the recovery action; startup and periodic reconciliation resume that exact lineage without duplicate wakes
- source-attempt exhaustion opens one source-scoped manager recovery action without changing the source assignee; manager runs use a separate persisted bounded retry counter and due time, then escalate visibly to the board when exhausted
